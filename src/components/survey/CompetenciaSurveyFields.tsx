...
interface Props {
  ...
  disabled?: boolean
  /** NR-01: permite editar o nome (sufixo de departamento etc.). Default false = Pentagrama. */
  editableName?: boolean
  /** Sufixo anexado ao nome gerado enquanto o usuário não “sujou” o campo (ex.: " — Fiscal"). */
  nameExtraSuffix?: string
}
...
export function CompetenciaSurveyFields({
  ...
  const surveyNameBase = parsed
    ? formatSurveyName(module, seq, parsed.month, parsed.year)
    : ''
  const suggestedName = surveyNameBase
    ? `${surveyNameBase}${nameExtraSuffix || ''}`
    : ''
  ...
  return (
    <>
        ...
        {editableName ? (
          <>
            <input
              ...
              className="block w-full rounded-lg border border-zinc-300 bg-white px-3 py-2.5 text-sm font-medium text-zinc-800"
              disabled={disabled}
            />
            <p className="text-xs text-zinc-500">
              Sugerido a partir da competência
              {nameExtraSuffix ? ' e do escopo de departamento' : ''}. Você pode editar.
            </p>
          </>
        ) : (
          <>
            <input type="hidden" name={nameField} value={surveyNameBase} readOnly />
            <input
              id="survey_name_preview"
              type="text"
              readOnly
              value={surveyNameBase || 'Preencha a competência (MM/AAAA)'}
      ...
      <fieldset className="space-y-4" disabled={disabled}>
        ...
        <p className="text-xs text-zinc-500">
          Prazo único da rodada — vale para toda a coleta (IL/IC no Pentagrama; NR-01 na janela de
          respostas).
        </p>
      </fieldset>
    </>
