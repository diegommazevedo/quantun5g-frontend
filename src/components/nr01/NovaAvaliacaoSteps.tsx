/**
 * Indicador de passos — nova avaliação NR-01.
 */
...
export function NovaAvaliacaoSteps({ step, companyName }: Props) {
  return (
    <nav aria-label="Progresso" className="rounded-xl border border-zinc-200 bg-white p-4">
      <ol className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-6">
        <li className="flex items-center gap-2">
          <span
            className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${
              step >= 1 ? 'bg-blue-800 text-white' : 'bg-zinc-200 text-zinc-600'
            }`}
          >
            1
          </span>
        ...
        <li className="flex items-center gap-2">
          <span
            className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold ${
              step >= 2 ? 'bg-blue-800 text-white' : 'bg-zinc-200 text-zinc-600'
            }`}
          >
            2
          </span>
