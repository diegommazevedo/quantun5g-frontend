...
const STAFF_SECTIONS: NavSection[] = [
  {
    ...
    label: 'Plataforma',
    items: [
      { href: '/', label: 'Início', icon: 'home', match: ['/', '/institucional'] },
      { href: '/pricing', label: 'Planos', icon: 'credit', match: '/pricing' },
    ],
  },
  {
    id: 'pentagrama',
...
export function buildNavSections(opts: {
  ...
  if (!staff) return LEADER_SECTIONS
  ...
  for (const section of STAFF_SECTIONS) {
    const items = section.items.filter((item) => {
      ...
      if (item.requiresModule === 'pentagrama' && !modulePentagrama) return false
      if (item.requiresModule === 'nr01' && !moduleNr01) return false
      if (section.id === 'empresas' && !modulePentagrama && !moduleNr01) return false
      return true
    })
